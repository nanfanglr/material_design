 ...
 * date: 2017-4-14
 *
 * @author 小嵩 https://blog.csdn.net/qq_22393017/article/details/70674044
 *             https://github.com/xiaosong520/EasyVoucherView
 * 实现的大致思路和步骤如下：
 * <p>
 * 定义一个VoucherView类继承自LinearLayout；
 * 在attrs.xml文件里面声明所需自定义属性；
 * 将自定义控件背景利用代码给设置成完全透明（详见VoucherView类的initDrawCanvas方法）；
 * 创建一个mCanvas画布并通过自定义属性BgColor获取需设定的颜色；
 * 创建一个mPaint画笔，用于绘制边缘锯齿；
 * 利用图像合成类PorterDuffXfermode并给Paint画笔设置DST_OUT模式；
 * 获取自定义属性drawType，根据自定义属性设定的形状，绘制边缘锯齿；
 * mCanvas画布擦掉mPaint画笔所绘制的形状部分。
 */
